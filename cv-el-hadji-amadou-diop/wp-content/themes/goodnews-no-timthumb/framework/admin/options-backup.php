...
class OptionsFramework_Backup {
	...
	function register_admin_screen () {
		...
		$this->admin_page = add_submenu_page('options-framework', __( 'Options Import / Export', 'framework' ), __( 'Import / Export', 'framework' ), 'manage_options', $this->token, array( &$this, 'admin_screen' ) );
	...
	<div class="wrap">
		...
		<div class="export">
			...
			<form method="post" action="<?php echo admin_url( 'admin.php?page=' . $this->token ); ?>">
				...
				<input type="submit" class="button" value="<?php _e( 'Download Export File', 'framework' ); ?>" />
			</form>
		</div>
		</div>
...
<?php
	...
	function admin_screen_help ( $contextual_help, $screen_id, $screen ) {
		...
		$contextual_help =
		  '<h3>' . __( 'Welcome to the Options Backup Manager.' ) . '</h3>' .
	...
	function admin_notices () {
		...
		echo '<div id="import-notice" class="updated"><p>' . sprintf( __( 'Please note that this backup manager backs up only your settings and not your content. To backup your content, please use the %sWordPress Export Tool%s.', 'framework' ), '<a href="' . admin_url( 'export.php' ) . '">', '</a>' ) . '</p></div><!--/#import-notice .message-->' . "\n";
		...
		if ( isset( $_GET['error'] ) && $_GET['error'] == 'true' ) {
			...
			echo '<div id="message" class="updated"><p>' . sprintf( __( 'Settings successfully imported. | Return to %sTheme Options%s', 'framework' ), '<a href="' . admin_url( 'admin.php?page=options-framework' ) . '">', '</a>' ) . '</p></div>';
		} // End IF Statement
