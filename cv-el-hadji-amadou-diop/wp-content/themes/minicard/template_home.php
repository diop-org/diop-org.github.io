...
<div id="mainContent" class="home_content">
		<?php the_content(); ?>
</div>
<?php endif; ?>
<?php endwhile; endif; ?>
...
<?php
	...
	//social_custom
	$custom = get_option('social_custom');
	$custom = explode("\n", $custom);
	if (sizeof($custom)>0) {
		foreach ($custom as $social_link) {
			$link_parts = explode('|', $social_link);
			$text = trim($link_parts[0]);
			$link = trim($link_parts[1]);
			$icon = trim($link_parts[2]);
			if (!empty($text) && !empty($link) && !empty($icon)) {
				$sort_social_links[] = $text;
				$social_links[] = array( 'name' => $text, 'link' => $link, 'icon' => $icon );
			}	
		}
	}
