...
function optionsframework_add_page() {
	...
	$of_page = add_menu_page(
		...
		OPTIONS_FRAMEWORK_URL.'images/options_icon.png',
		61
		);
...
function optionsframework_load_scripts() {
	...
	wp_enqueue_script('iphone-style', OPTIONS_FRAMEWORK_URL.'js/iphone-style-checkboxes.js', array('jquery'));
	wp_enqueue_script('jquery-tools	', OPTIONS_FRAMEWORK_URL.'js/jquery.tools.min.js', array('jquery'));
	wp_enqueue_script('Tipsy', OPTIONS_FRAMEWORK_URL.'js/jquery.tipsy.js', array('jquery'));
...
function optionsframework_page() {
	...
	<div class="left">
	<div class="logo">
		<img src="<?php echo $imagepath  ?>logo.png" alt="Momiza Panel">
	</div> <!--End Logo-->
    <ul class="mom-tab-wrapper">
        <?php echo $return[1]; ?>
    </ul>
	</div> <!--End Left-->
	<div class="right">
		<div class="links">
			...
			<div class="theme_meta">
				<div class="theme_img">
			<img src="<?php echo MOM_SCRIPTS ?>/timthumb.php?src=<?php echo MOM_URI; ?>/screenshot.png&amp;h=87&amp;w=120&amp;zc=1" alt="theme" />
				</div>
				<ul class="theme_links">
					<li><a href="<?php echo home_url(); ?>" target="_blank"><?php _e('Home', 'framework') ?></a></li>
					<li><a href="<?php echo MOM_URI; ?>/framework/docs" target="_blank"><?php _e('Documentatihon', 'framework') ?></a></li>
					<li><a href="http://support.momizat.net/" target="_blank"><?php _e('Support', 'framework') ?></a></li>
				</ul>
			</div>
			</div>
		</div><!--End Links-->
    <div class="framework_wrap">
	...
	<div class="clear" style="border-bottom:1px solid #000a25;"></div>
	        <div id="optionsframework-submit">
			<input type="submit" class="fr_bt" name="update" value="<?php esc_attr_e( 'Save Options' ); ?>" />
            <input type="submit" class="reset-button fr_bt" name="reset" value="<?php esc_attr_e( 'Restore Defaults' ); ?>" onclick="return confirm( '<?php print esc_js( __( 'Click OK to reset. Any theme settings will be lost!' ) ); ?>' );" />
            <div class="clear"></div>
	    		</div>
			</form>
...
function optionsframework_validate( $input ) {
	...
	if ( isset( $_POST['update'] ) ) {
		...
		foreach ( $options as $option ) {
			...
			if ( 'checkut' == $option['type'] && ! isset( $input[$id] ) ) {
				$input[$id] = '0';
			}
			...
			// For a value to be submitted to database it must pass through a sanitization filter
						if ($option['type'] == 'textarea' || $option['type'] == 'editor' || $option['type'] == 'range') {
