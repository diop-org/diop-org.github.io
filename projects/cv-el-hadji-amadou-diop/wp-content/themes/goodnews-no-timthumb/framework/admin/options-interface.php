...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		// Checkbox
		case "checkbox":
			$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="checkbox iphonecheck of-input" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" '. checked( $val, 1, false) .' />';
		break;

		// Checkboxut
		case "checkut":
			$output .= '<input id="' . esc_attr( $value['id'] ) . '" class="checkbox iphonecheck of-input checkut" type="checkbox" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" '. checked( $val, 1, false) .' />';
		break;
		...
		case "heading":
			...
			$jquery_click_hook = preg_replace('/[^a-zA-Z0-9._\-]/', '', strtolower($value['slug']) );
			$jquery_click_hook = "of-option-" . $jquery_click_hook;
			$menu .= '<li><a id="'.  esc_attr( $jquery_click_hook ) . '-tab" class="navi-tab ' . esc_attr( $value['slug'] ) . '" title="' . esc_attr( $value['name'] ) . '" href="' . esc_attr( '#'.  $jquery_click_hook ) . '">' . esc_html( $value['name'] ) . '</a></li>';
		...
		case 'editor':
			...
			$output .= '<textarea id="'. $value['id'] .'" class="of-input wysiwyg-editor" name="'.$option_name.'['.$value['id'].']" cols="'. $cols .'" rows="12">'.wpautop($val).'</textarea>';
		break;
		*/
		//Range
		case 'range':
			$output .= '<input class="range" id="'. $value['id'] .'" type="range" name="'.$option_name.'['.$value['id'].']" value="'. $val .'"';
			if (isset($value['min'])) {
		...
		if (isset($value['suffix'])) {
			$output .= '<span class="space">'. $value['suffix'] .'</span>';
		}
				break;
		// Basic text input
		case 'img':
			$output .= '<div class="img_ref"><img  class="of_img" src="' . $value['src'] . '" ></div>';
		break;
		...
		if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
			...
			$output .= '</div>';
			if ( $value['desc'] != "" ) {
				$output .= '<div class="explain" title="' . esc_html( $explain_value) . '"></div>'."\n";
			}
