...
<style type="text/css">
/*Primary Color*/
#topmenu ul .current-menu-parent ul .current-menu-item, #topmenu ul li ul li a:hover, #topmenu ul li .sub-menu .current-menu-item a:hover, #slide_wrap, #lay1 .post, #lay2 .post, #slide_acord, #lay4 .post, .post, .type-page, .type-attachment, .amp_page, .amp_next, .amp_prev, .page-numbers, .comment-body, #respond, .share_this, #related {background:#3e3e3e url(<?php echo get_template_directory_uri(); ?>/images/<?php echo $option['scl_patterns'] ?>.png) repeat!important;}

/*Secondary Color*/
#topmenu ul li:hover, #topmenu ul li.current-menu-item, #topmenu ul .current-menu-item ul li, #topmenu ul li ul li, #prevBtn, #nextBtn, #lay1 .date, #lay2 .block_comm, .post .date, .type-page .date, .type-attachment .date, .post tr th, .post thead th, #posts .sticky, .more-link, .amp_current, .amp_page:hover, .amp_next:hover, .amp_prev:hover, .page-numbers:hover, .navigation .current, #respond #submit, .bypostauthor .comment-body, #related h3, .entry-content #submit_msg, .trigger_wrap, .scl_button { background:#BF4842 url(<?php echo get_template_directory_uri(); ?>/images/<?php echo $option['scl_patterns'] ?>.png) repeat!important; }

.post .post_content a, .page .post_content a{color:#BF4842!important;}
/*Widgets*/
#sidebar ul .widget{ background:#BF4842 url(<?php echo get_template_directory_uri(); ?>/images/<?php echo $option['scl_patterns'] ?>.png) repeat!important; }

#sidebar .widgettitle{background:#333333 url(<?php echo get_template_directory_uri(); ?>/images/<?php echo $option['scl_patterns'] ?>.png) repeat;}
...
.ribbon{ color:#ffffff!important; background:#BF4842 url(<?php echo get_template_directory_uri(); ?>/images/<?php echo $option['scl_patterns'] ?>.png) repeat!important;}

/*THE DATE BADGE*/
.post .date, .type-page .date, .type-attachment .date, #lay1 .date, #lay2 .date{ color:#6E2926!important; background:#BF4842 url(<?php echo get_template_directory_uri(); ?>/images/<?php echo $option['scl_patterns'] ?>.png) repeat!important;}
