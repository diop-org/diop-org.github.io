...
<?php if ( have_posts() ) while ( have_posts() ) : the_post(); ?>
				...
				<div id="nav-above" class="navigation">
					<div class="nav-previous"><?php previous_post_link( '%link', '<span class="meta-nav">' . _x( '&larr;', 'Previous post link', 'absolum' ) . '</span> %title' ); ?></div>
					<div class="nav-next"><?php next_post_link( '%link', '%title <span class="meta-nav">' . _x( '&rarr;', 'Next post link', 'absolum' ) . '</span>' ); ?></div>
				</div><!-- #nav-above -->
...
<?php if ( get_the_author_meta( 'description' ) ) : // If a user has filled out their description, show a bio on their entries  ?>
					<div id="entry-author-info">
						...
						<div id="author-description">
							<h2><?php printf( esc_attr__( 'About %s', 'absolum' ), get_the_author() ); ?></h2>
							<?php the_author_meta( 'description' ); ?>
							<div id="author-link">
								<a href="<?php echo get_author_posts_url( get_the_author_meta( 'ID' ) ); ?>">
									<?php printf( __( 'View all posts by %s <span class="meta-nav">&rarr;</span>', 'absolum' ), get_the_author() ); ?>
								</a>
							</div><!-- #author-link	-->
