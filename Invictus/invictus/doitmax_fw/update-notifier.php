...
define( 'NOTIFIER_THEME_NAME', 'Invictus' ); // The theme name
define( 'NOTIFIER_THEME_FOLDER_NAME', 'invictus' ); // The theme folder name
define( 'NOTIFIER_XML_FILE', 'http://themes.do-media.de/tf_themes/wordpress/invictus/notifier_invictus.xml' ); // The remote notifier XML file containing the latest version of the theme and changelog
...
function update_notifier_menu() {  
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			$new_update = '<span class="update-plugins count-1"><span class="update-count">New Update!</span></span>';
		}else{
			$new_update = "";
		}
		add_dashboard_page( NOTIFIER_THEME_NAME . ' Theme Updates', NOTIFIER_THEME_NAME . $new_update, 'administrator', 'theme-update-notifier', 'update_notifier');
	}	
}
...
function update_notifier_bar_menu() {
	if (function_exists('simplexml_load_string')) { // Stop if simplexml_load_string funtion isn't available
		...
		if( (float)$xml->latest > (float)$theme_data['Version']) { // Compare current theme version with the remote XML version
			$new_update = '<span id="ab-updates">New Update!</span>';
		}else{
			$new_update = "";
		}
		$wp_admin_bar->add_menu( array( 'id' => 'update_notifier', 'title' => '<span>' . NOTIFIER_THEME_NAME . $new_update. '</span>', 'href' => get_admin_url() . 'index.php?page=theme-update-notifier' ) );
	}
}
...
function update_notifier() {
	...
	if( ini_get('allow_url_fopen') ){
		...
		<div class="wrap">
			...
			?>	    
			<br />
			<div id="message" class="error below-h2"><p><strong>There is a new version of the <?php echo NOTIFIER_THEME_NAME; ?> theme available.</strong> You have version <?php echo $theme_data['Version']; ?> installed. <strong>Please update to version <?php echo $xml->latest; ?>.</strong></p></div>
			<br />			
			<?php }else{ 
			// No new version available
			?>		
			<br />
			<div id="message" class="updated below-h2"><p><strong>Your theme is up to date.</strong> The latest version <?php echo $theme_data['Version']; ?> of <?php echo NOTIFIER_THEME_NAME; ?> theme is installed.</strong></p></div>
			<br />		
			<?php  } ?>
...
function get_latest_theme_version($interval) {
	...
	$db_cache_field_themename = 'notifier-cache-theme-name';	
	$last = get_option( $db_cache_field_last_updated );
	$theme = get_option ( $db_cache_field_themename);
	
	$now = time();
	// check the cache
	if ( !$last || (( $now - $last ) > $interval) || !$theme ) {
